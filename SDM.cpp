...
SDM::SDM(HardwareSerial& serial, long baud, int tx_pin, int rx_pin, int dere_pin, int config, bool swapuart) : sdmSer(serial) {
  this->_baud = baud;
  this->_config = config;
...
void SDM::begin(void) {
  ...
  #else
    sdmSer.begin(_baud, _config, _tx_pin, _rx_pin);
  #endif
#else
  sdmSer.begin(_baud);
#endif
