...
//------------------------------------------------------------------------------
  ...
  #define SDM_UART_BAUD                     2400                                //default baudrate
#endif
...
class SDM {
  public:
#ifdef USE_HARDWARESERIAL
    SDM(HardwareSerial& serial, long baud = SDM_UART_BAUD, int tx_pin = TX_PIN, int rx_pin = RX_PIN, int dere_pin = DERE_PIN, int config = SDM_UART_CONFIG, bool swapuart = SWAPHWSERIAL);
#else
